...
def multifru_net():
    ...
    frulist = subprocess.run(['/usr/bin/ipmitool', '-I', 'lanplus', '-H', ip, '-U',login, '-P', pwd, 'fru', 'list'], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding='utf-8')
    print(frulist.stdout)
    checkback = input('Всё верно?(y/n): ')
    if checkback == 'n':
...
def local_srv(): #Функция локального обновления FRU
    if com == ('n'):
        process = subprocess.run(['/usr/bin/ipmitool', '-L', '-U',login, '-P', pwd, 'shell'])
    elif com == ('y'):
        AT = str(input('Asset_tag: '))
        process = subprocess.run(['/usr/bin/ipmitool', '-L', '-U', login, '-P', pwd, 'fru', 'edit', '0', 'field', 'p', '5', AT], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding='utf-8')
